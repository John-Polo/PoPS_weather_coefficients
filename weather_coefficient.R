...
weather_coefficient <- function(directory, output_directory, start, end, time_step, study_area = "states", states_of_interest= c('California'), 
  ...
  if(prcp_index == 'YES'){
    ...
    prcp <- rast() # Create raster stack for the area of interest and years of interest from Daymet data
    if(future_scenarios == 'NO'){
      dates <- str_sub(precip_files,16,19) # Assumes daymet data is saved in the exact naming format that it is downloaded as
  ...
  if(temp_index == 'YES'){
    ...
    if(future_scenarios == 'NO'){
      dates <- str_sub(tmax_files,16,19) # Assumes daymet data is saved in the exact naming format that it is downloaded as
  ...
  if(lethal_temperature == 'YES') {
    lethal_temp_stack = rast()
  }
  
  ## reference shapefile used to clip, project, and resample 
  if (study_area == "states"){
    states <- st_read("H:/Shared drives/Data/Vector/USA/us_lower_48_states.gpkg") # link to your local copy
  ...
  for (i in 1:total_years) {
    ## Precipitation 
    if(prcp_index == 'YES'){
      precip <- rast(paste(directory,"/",precip_files[[i]], sep = ""), varname = "prcp", quick = TRUE)
      precip3 <- crop(precip[[1]],reference_area)
      precip2 <- rast(precip3)
      for(ind in 2:nlyr(precip)){
        precip3 <- crop(precip[[ind]], reference_area)
        precip2 <- rast(precip2, precip3)
      }
      if (i>1 && compareGeom(precip,prcp) == FALSE) { precip@crs <- crs(prcp) }
      prcp <- rast(prcp, precip2)
      rm(precip, precip2, precip3)
    }

    ## Temperature
    if(temp_index == 'YES'){
      tmin <- rast(paste(directory, "/", tmin_files[[i]], sep =""), varname = "tmin", quick = TRUE)
      tmin3 <- crop(tmin[[1]], reference_area)
      tmin2 <- rast(tmin3)
      if (i>1 && compareGeom(tmin2,tmin_s) == FALSE) { tmin@crs <- crs(tmin_s) }
      tmax <- rast(paste(directory, "/", tmax_files[[i]], sep = ""), varname = "tmax", quick = TRUE)
      tmax3 <- crop(tmax[[1]], reference_area)
      tmax2 <- rast(tmax3)
      tavg2 <- (tmin3+tmax3)/2
      tavg <- rast(tavg2)
      if (i>1 && compareGeom(tmax2,tmax_s) == FALSE) { tmax@crs <- crs(tmax_s) }
      for (ind2 in 2:nlyr(tmin)){
        tmin3 <- crop(tmin[[ind2]], reference_area)
        tmax3 <- crop(tmax[[ind2]], reference_area)
        tmin2 <- rast(tmin2, tmin3)
        tmax2 <- rast(tmax2, tmax3)
        tavg2 <- (tmin3+tmax3)/2
        tavg <- rast(tavg, tavg2)
      }
      # tavg <- tmax2
      # for (j in 1:nlyr(tmax)){
      #   tavg[[j]] <- overlay(tmax2[[j]], tmin2[[j]], fun = function(r1, r2){return((r1+r2)/2)})
      # }
      tmin_s <- rast(tmin_s, tmin2)
      tmax_s <- rast(tmax_s, tmax2)
      tavg_s <- rast(tavg_s, tavg)
      if (lethal_temperature == 'YES') {
        lethal_index <- which(str_sub(names(tmin), 7,8) == lethal_month)
        lethal_rast <- tavg[[lethal_index]]
        if(lethal_min == 'YES') {
          lethal_temp <- tapp(lethal_rast, indices = rep(1,nlyr(lethal_rast)), fun=min)
        } else {
          lethal_temp <- tapp(lethal_rast, indices = rep(1,nlyr(lethal_rast)), fun=max)
        } 
        lethal_temp_stack <- rast(lethal_temp_stack, lethal_temp)
      }
  ...
  if (prcp_index =='YES') {
    if(time_step == "daily"){
      ...
      timestep_names <-  names(prcp[[seq(1,nlyr(prcp),7)]])
      indices <- rep(seq(1,((nlyr(prcp)/7)+1),1),7)
      indices <- indices[1:nlyr(prcp)]
    ...
    if(time_step == "daily"){
      ...
      timestep_names <-  names(prcp[[seq(1,nlyr(prcp),7)]])
      indices <- rep(seq(1,((nlyr(tavg_s)/7)+1),1),7)
      indices <- indices[1:nlyr(tavg_s)]
      indices <- indices[order(indices)]
  ...
  if (prcp_index == 'YES' && prcp_method == "reclass"){
    prcp_coeff <- classify(prcp,prcp_matrix)
    prcp_coeff <- tapp(prcp_coeff, indices, fun=mean)
    names(prcp_coeff) <- timestep_names
  }
  
  if (temp_index == 'YES' && temp_method == "reclass"){
    temp_coeff <- classify(tavg_s,temp_matrix)
    temp_coeff <- tapp(temp_coeff, indices, fun=mean)
  ...
  if (prcp_index == 'YES' && prcp_method == "polynomial"){
    prcp_coeff <- tapp(prcp, indices, fun=mean)
  ...
  if (temp_index == 'YES' && temp_method == "polynomial"){
    temp_coeff <- tapp(tavg_s, indices, fun=mean)
  ...
  if(future_scenarios == 'YES'){
    if(prcp_index == 'YES') {
      prcp_coeff_ranking <- tapp(prcp_coeff, indices = indices_weather_ranking, fun = mean)
    }
    if(temp_index == 'YES') {
      temp_coeff_ranking <- tapp(temp_coeff, indices = indices_weather_ranking, fun = mean)
    }
    if(lethal_temperature == 'YES') {
      ...
      lethal_temp_area <- classify(lethal_temp_stack, lethal_temp_matrix)
    }
    
    first_year <- as.numeric(str_sub(precip_files[[1]],16,19))
    last_year <- as.numeric(str_sub(precip_files[[total_years]],16,19))
    ...
    for (i in 1:total_years) {
      ...
      weather_ranking$avg_temp_coeff[i] <- mean(values(temp_coeff_ranking[[i]]), na.rm = TRUE)
      weather_ranking$avg_prcp_coeff[i] <- mean(values(prcp_coeff_ranking[[i]]), na.rm = TRUE)
    }
